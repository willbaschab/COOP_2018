...
def grayscale(win, img):
    """
    This method handles the transformation of an image into a grey image and returns the new image back to caller
    ...
    - for each row in in the range of rows and for each column in the range of columns
        ...
        - set the new value of gray to the formula int((max(rgb) + min(rgb)) / 2)
        - set the pixel of same row and column to have the rgb value of (graylit, graylit, graylit)
    ...
    for row in range(rows):
        for column in range(columns):
            ...
            img.setPixel(row, column, color_rgb(graylit, graylit, graylit))
...
def main():
    ...
    - In while True state:
        ...
        - if the mouse variable returns anything:
            ...
            - if click was in remove box:
                ...
                - set the greyscale_state to 0 (with last image removed, the next one will not have been grayed yet)
            ...
            - if click was in greyscale box and the photo has not been greyscaled yet:
                - turn all buttons grey while this oen is active

                - set photo equal to greyscale() method (it returns the photo after editing it)

                - add one to the greyscale_state (so that image can't be greyscaled twice)
    ...
    # Main Variables
    titles = ['Upload Photo', 'Remove Photo', 'Greyscale Photo', 'Save Photo']
    colors = ['white', 'gray', 'gray', 'gray']
    # buttons[0] is upload box
    # buttons[1] is remove box
    # buttons[2] is greyscale box
    upload_state = 1
    greyscale_state = 0
    save_state = 1
    photo_state = 0
    ...
    while True:
        ...
        if mouse:
            ...
            elif check_option(mouse, buttons[2], photo_state, 1) and greyscale_state % 2 == 0:  # if in save text box
                # and the image has not been grayscaled yet (to prevent it from happening again)
                for i in range(1, 4):
                    change_color(buttons[i], 'gray')

                photo = grayscale(win, photo)
